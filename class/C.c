...
//   }
// }
// void withdrawAmount(struct Customer c[], int N, int accNo, int amount) {
//     for (int i=0;i<N;i++){
//       if (accNo==c[i].accNo){
//         printf("Existing Balance: %d\n",c[i].balance);
//         int bal=0;
//         if (c[i].balance>amount){
          
//           bal=c[i].balance-amount;
...
// // Function to print customers with balance < 100
// void printLowBalance(struct Customer c[], int n) {
//     for(int i = 0; i < n; i++) {
//         if(c[i].balance < 100) {
//             printf("%s %d\n", c[i].name, c[i].accNo);
...
// void withdrawAmount(struct Customer c[], int n, int acc, int amount) {
//     for(int i = 0; i < n; i++) {
//         if(c[i].accNo == acc) {
//             printf("Account Number: %d\n", acc);
//             printf("Existing Balance: %d\n", c[i].balance);
...
function processOrders(arr){
    ...
    if(arr[i].delivered==true){
      total+=arr[i];
    }  
    }
    let count=0;
    for(let i=0;i<=arr.length;i++){
      if (arr[i].delivered==false){
        count++;
      }
    }
    let highvalue=[];
    for (let i=0;i<=arr.length;i++){
      if (arr[i].amount>1000){
        highvalue.push(arr[i].ordered);
        
      }
    }
    
    return(total,count,highvalue);
  }

  
  function processOrders(arr){
    // Total revenue from delivered orders
    let totalDeliveredRevenue = arr
        .filter(order => order.delivered)
        .reduce((sum, order) => sum + order.amount, 0);

    // Count pending orders
    let pendingCount = arr
        .filter(order => !order.delivered)
        .length;

    // Order IDs with amount > 1000
    let highValueOrderIds = arr
        .filter(order => order.amount > 1000)
        .map(order => order.orderId);

    return [totalDeliveredRevenue, pendingCount, highValueOrderIds];
}